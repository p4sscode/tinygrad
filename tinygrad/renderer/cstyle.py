...
base_rewrite = PatternMatcher([
  ...
  (UPat(Ops.VCONST, name="x"),
   lambda ctx,x: f"{ctx.float4.replace('float4', ctx.render_dtype(x.dtype))}" + \
    (f"{{{','.join([ctx.string_rewrite.rewrite(x.const_like(y), ctx=ctx) for y in x.arg])}}}" if ctx.device == "CLANG" else
      f"({','.join([ctx.string_rewrite.rewrite(x.const_like(y), ctx=ctx) for y in x.arg])})")),
  # new load/store
