...
class CUDARenderer(CStyleLanguage):
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None):
    ...
    for name, (N, M, K), dtype_in, dtype_out, _, _, upcast_axes, _ in dedup([uop.arg for uop in uops if uop.op is UOps.WMMA]):
      buf_upcs = tuple(prod(sz for _, sz in upc) for upc in upcast_axes)
      buf_dtypes = tuple(self.render_dtype(dt.vec(upc)) for dt,upc in zip([dtype_in,dtype_in,dtype_out],buf_upcs))
      buf_szs = tuple(upc*dt.itemsize//4 for dt,upc in zip([dtype_in,dtype_in,dtype_out],buf_upcs))
      operands = [f"%{i}" for i in range(sum(buf_szs))]
      prefix.append(f"""__device__ {buf_dtypes[2]} __{name}({buf_dtypes[0]} a, {buf_dtypes[1]} b, {buf_dtypes[2]} c){{
  int *a_pk = (int *)(&a), *b_pk = (int *)(&b);\n  asm("mma.sync.aligned.m{M}n{N}k{K}.row.col.f32.{dt_map[dtype_in]}.{dt_map[dtype_in]}.f32"
      "{{{",".join(operands[:buf_szs[2]])}}},\n    {{{",".join(operands[buf_szs[2]:buf_szs[2]+buf_szs[0]])}}}, 
       {{{",".join(operands[buf_szs[2]+buf_szs[0]:])}}},\n    {{{",".join(operands[:buf_szs[2]])}}};"
    : {", ".join([f'"+f"(c.{_nms[i]})' for i in range(buf_szs[2])])}
    : {", ".join([f'"r"(a_pk[{i}])' for i in range(buf_szs[0])])}, {", ".join([f'"r"(b_pk[{i}])' for i in range(buf_szs[1])])});\n  return c;\n}}""")
