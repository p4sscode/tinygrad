...
class CStyleLanguage(Renderer):
  ...
  def render_const(self, x:ConstType, dtype:DType) -> str:
    ...
    return (self.render_vectorize([val] * dtype.count, dtype) if dtype.count > 1 or dtype not in [dtypes.float, dtypes.int, dtypes.bool] else val)
      ...
      if uop is UOps.IF:
        ...
        if uop is UOps.RANGE:
          ...
          if uop is UOps.BITCAST:
            ...
            val = self.render_cast(r[src[0]], dtype, bitcast=False)
          if child_count[u] <= 1: r[u] = val
          else: kk(f"{self.render_dtype(dtype)} {ssa('cast',u)} = {val};")
        elif uop is UOps.VECTORIZE:
          val = self.render_vectorize([r[x] for x in src], dtype)
          if child_count[u] <= 1: r[u] = val
          else: kk(f"{self.render_dtype(dtype)} {ssa('cast',u)} = {val};")
