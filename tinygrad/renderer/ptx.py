...
class PTXRenderer(Renderer):
  ...
  tc_sm80, tc_sm75 = [tc for tc in CUDARenderer.tc_sm80 if tc.dtype_in == dtypes.half], CUDARenderer.tc_sm75
  code_for_op = asm_for_op
  extra_matcher = ptx_matcher
  def __init__(self, arch:str, device="CUDA"):
    self.device, self.arch = device, arch
    self.tensor_cores = PTXRenderer.tc_sm80 if int(arch[3:]) >= 80 else PTXRenderer.tc_sm75 if int(arch[3:]) >= 75 else []
