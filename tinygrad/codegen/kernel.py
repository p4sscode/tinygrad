    ...
    if use_tensor_cores and self.reduceop is not None and self.reduceop.arg[0] is BinaryOps.ADD:
      for tc in self.opts.tensor_cores:
        ...
        for opt in tc.opt_seq:
          ...
          if opt == "UP": [self.apply_opt(Opt(OptOps.UPCAST,tc_opts.axes[tc_dim],amt), append_opt=False) for tc_dim,amt in tc.early_upcast_axes]
          if opt == "LC": [self.apply_opt(Opt(OptOps.LOCAL,tc_opts.axes[tc_dim],amt), append_opt=False) for tc_dim,amt in tc.threads]
        self.tensor_core = tc
