...
def full_graph_rewrite(sink:UOp, opts:Optional[Renderer]=None) -> UOp:
  ...
  if linearize_cnt != (de:=getenv("DEBUG_EXPAND", 0)) and de != -1:
    ...
    if getenv("DO_REDUCE", 1):
      ...
      ops_from_keys = tuple(dedup(op for op,_ in opts.code_for_op.keys())) if opts is not None else ()
      sink = graph_rewrite(sink, sym+get_extra_patterns(ops_from_keys, TRANSCENDENTAL>=2))
