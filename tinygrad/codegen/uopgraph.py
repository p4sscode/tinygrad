...
def full_graph_rewrite(sink:UOp, opts:Optional[Renderer]=None) -> UOp:
  ...
  if linearize_cnt != (de:=getenv("DEBUG_EXPAND", 0)) and de != -1:
    ...
    if getenv("DO_REDUCE", 1):
      ...
      sink = graph_rewrite(sink, sym+get_extra_patterns(tuple(key[0] if isinstance(key, tuple) else key for key in opts.code_for_op.keys()) if opts is not None else (), TRANSCENDENTAL>=2))
