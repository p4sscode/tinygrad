...
from tinygrad.dtype import dtypes, PtrDType, ImageDType, is_dtype_supported
...
def full_graph_rewrite(sink:UOp, opts:Optional[Renderer]=None) -> UOp:
  ...
  if opts is not None and opts.extra_matcher is not None:
    ...
    if not is_dtype_supported(dtypes.bfloat16, opts.device):
      sink = graph_rewrite(sink, bf16_pm)
      sink = graph_rewrite(sink, bf16_uop_casting_matcher)
  return sink
