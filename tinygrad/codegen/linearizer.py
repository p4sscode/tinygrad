...
class Linearizer(Kernel):
    ...
    for idx, valid, rep_idx in zip(e_idxs, e_valids, iter_idxs(expand_vars)):
      ...
      if key not in self.load_cache:
        if acc is not None:
          ...
          rendered_idx = UOp(UOps.VECTORIZE, dto:=dtypes.int.vec(2), tuple(x.render(render_ops, self.loop_uops) for x in image_idx)).cast(dto)
  ...
  def global_store(self, i:int, idxs:List[Node], store:List[UOp]) -> List[UOp]:
    ...
    if len(upcast_dim := self.get_float4_upcast_dim(i)) == 1 and len(float4_expand := expand_node(idxs[upcast_dim[0]])) in [2,4]:
      ...
      for k,grouped in grouped_store_offset.items():
        ...
        store_offset_new[k] = UOp(UOps.VECTORIZE, dto:=buf.dtype.vec(amt), tuple(grouped)).cast(dto)
    ...
    for _idx, var in store_offset.items():
      ...
      if isinstance(buf.dtype, ImageDType):
        image_idx, valid = to_image_idx(buf.dtype.shape, idx, valid)
        rendered_idx = UOp(UOps.VECTORIZE, dto:=dtypes.int.vec(2), \
                      tuple(x.render(render_ops, self.loop_uops) for x in image_idx)).cast(dto)
      else:
        rendered_idx = idx.render(render_ops, self.loop_uops)
  ...
  def render_reduceop(self, reduceop:LazyOp, accs:Dict[LazyOp, List[UOp]], loaded_buffers:Dict[Union[MemBuffer, ConstBuffer, LocalBuffer], List[UOp]],
    ...
    if (tc:=self.tensor_core):
      ...
      # cast initial accs
      wmmas = [UOp(UOps.VECTORIZE, (dt3:=tc.dtype_out.vec(wmma_sz[2])), tuple(accs[reduceop][x:x+wmma_sz[2]])).cast(dt3)
      ...
      for it in [x[::-1] for x in itertools.product(*list([range(sz) for _,sz in upcasts[0]][::-1]))]:
        ...
        ops = (UOp(UOps.VECTORIZE, dti0:=tc.dtype_in.vec(wmma_sz[0]), tuple(locals_to_store[0][2][offs[0]:offs[0]+wmma_sz[0]])).cast(dti0),
                UOp(UOps.VECTORIZE, dti1:=tc.dtype_in.vec(wmma_sz[1]), tuple(locals_to_store[1][2][offs[1]:offs[1]+wmma_sz[1]])).cast(dti1),
